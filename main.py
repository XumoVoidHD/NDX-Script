            ...
            if (start_time <= current_time <= closing_time) or self.testing:
                ...
                if credentials.instrument == "NDX":
                    if str(int(closest_strike)).endswith("75"):
                        closest_strike = (closest_strike // 10) * 10 + 70
                    elif str(int(closest_strike)).endswith("25"):
                        closest_strike = (closest_strike // 10) * 10 + 20
    ...
    async def call_hedge_check(self):
        while self.should_continue:
            if self.call_order_placed:
                ...
                call_exists = any(
                    trade.contract.secType == 'OPT' and trade.contract.right == 'C' and trade.contract.symbol == "NDX"
    ...
    async def put_hedge_check(self):
        while self.should_continue:
            if self.put_order_placed:
                ...
                put_exists = any(
                    trade.contract.secType == 'OPT' and trade.contract.right == 'P' and trade.contract.symbol == "NDX"
                    for trade in open_trades
                )
